...
import { Button, Form, Input, Icon, Select, Tabs, DatePicker, Menu, Dropdown, Modal, Radio, Upload, Progress, notification, message, Tooltip, Card } from 'antd';
...
//          const { permission } = this.state;
          switch (record.order_status) {
            ...
            case '1':
              ...
              //发货信息填写
              permission['100013'].status && !record.riskStatus ? result.splice(1,0,{ title: languageForOrder.Shipping, key: 4 }) : '';
              //标记为风险订单
    ...
    return (
      <div id={styles.orderListBox}>
        ...
        <div className={styles.orderListTable}>
          <Card>
            <Form layout="inline" onSubmit={this.handleSubmit} className="margin-bottom-24">
              <div className={styles.searchBox}>
                <div>
                      ...
                      >
                        {
                          selectSearchDatas.map((items) => {
                            return <Option value={items.key} key={items.key} title={items.value}>{items.value}</Option>;
                          })
                        }
                      </Select>
                    )}
                  ...
                  {
                    permission['100014'].status && ['-1','6'].indexOf(orders.pushData.tab_id.toString())=== -1 ? (<FormItem>
            ...
            <Form>
              ...
              >
                {getFieldDecorator('type', {
                  rules: [{type: 'string', required: true, message: `${languageForGlobal.PleaseEnter}${languageForOrder.ShipopingInfomation}`, whitespace: true}],
                  initialValue: "",
                })(
                  <Select>
                    {
              ...
              >
                {getFieldDecorator('links', {
                  rules: [{type: 'string', required: true, message: `${languageForGlobal.PleaseEnter}${languageForOrder.LogisticsTrackingURL}`, whitespace: true}],
                  initialValue: "",
                })(
              ...
              >
                {getFieldDecorator('deliveryTime', {
                  rules: [{ required: true, message: `${languageForGlobal.PleaseEnter}${languageForOrder.ShippingTime}`}],
        ...
        {/*<Modal*/}
          {/*title={ '修改发货信息'}*/}
          {/*visible={this.state.modifyShippingVisble}*/}
          {/*onOk={this.modifyShippingInfo}*/}
          {/*onCancel={()=>{*/}
          {/*this.setState({*/}
          {/*modifyShippingVisble: false*/}
          {/*})*/}
          {/*}}*/}
          {/*>*/}
          {/*<Form>*/}
          ...
          {/*rules: [{ required: false}],*/}
          {/*})(*/}
          {/*<TextArea placeholder={modifyShippingParams.shippingNo} rows={4} />*/}
          {/*)}*/}
          {/*</FormItem>*/}
          {/*<p style={{fontSize:12}}>*/}
          {/*1、“确认修改”后，会邮件通知用户发货信息已更新；<br/>*/}
          {/*2、如订单包含多个包裹，可一次性导入多个物流单号。*/}
          {/*</p>*/}
          {/*</Form>*/}
          {/*</Modal>*/}
        </div>
