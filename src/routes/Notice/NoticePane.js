          ...
          <FormItem>
            {getFieldDecorator('searchType', {
              ...
              <Select
                ...
                disabled={searchType[type].length <= 1}
              >
                {
                  searchType[type].map((items) => {
                    return <Option value={items.id} key={items.id}>{items.text}</Option>;
                  })
                }
              </Select>
            )}
          ...
          resource.length > 0 ? resource.map((item) => {
            return (
              <div key={`${item.id}-notice`}>
                <div className={`${styles.orderPaneList} ${item.isRead == 1 ? styles.invalid : ''}`} onClick={
                  () => {
                    if (item.isRead == 0) {
                      this.props.onClick(item.id)
                    }
            ...
            lineHeight: '100px',
            marginTop:54
          }}>{languageForProductNotice.NoNotificationNow}</div>)
        }
      </div>
    )
  }
