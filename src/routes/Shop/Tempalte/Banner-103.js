...
export default class Banner103 extends Component {
  ...
  //
  bannerInputHandleSpuId = (e) => {
    const {selectedCard} = this.props;
    const val = e.target.value
    selectedCard.info.spuId = val
    this.props.onSetState('selectedCard',selectedCard);
    this.forceUpdate();
  }

  bannerInputHandleViewId = (e) => {
    const {selectedCard} = this.state
    const val = e.target.value
    selectedCard.info.viewId = val
    this.props.onSetState('selectedCard',selectedCard);
    this.forceUpdate();
  }
  //
  bannerInputHandleUrl = (e) => {
    const {selectedCard} = this.props;
    const val = e.target.value
    selectedCard.info.url = val
    this.props.onSetState('selectedCard',selectedCard);
    this.forceUpdate();
  }
  //更换banner
          ...
          >
            ...
            <div style={{display: 'inline-block'}}>
              ...
              {resource.info.type === 1 && <Input
                ...
                onBlur={this.bannerInputHandleSpuId}/>
              }
              {
                resource.info.type === 2 && <Input
                  ...
                  onBlur={this.bannerInputHandleViewId}/>
              }
              {
                resource.info.type === 3 && <Input
                  ...
                  onBlur={this.bannerInputHandleUrl}/>
              }
            </div>
          </FormItem>
        </div>
