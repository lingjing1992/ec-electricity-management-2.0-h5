...
export default {
  ...
  effects: {
    // 获得优惠码列表
    * couponSubtractionGetDiscounts({payload, callback}, {call, put}) {
    ...
    //创建优惠码
    * couponSubtractionCreateDiscount({payload, callback}, {call, put, select}) {
      console.log('payload', payload);
      ...
      if (response.status === 200) {
        yield put(routerRedux.push('/marketing/couponSubtractionList'));
    ...
    //更新有效期
    * couponSubtractionUpdateExpireDate({payload, callback}, {call, put}) {
      console.log('payload', payload);
    ...
    //更新编辑商品
    * couponSubtractionUpdateEditProduct({payload, callback}, {call, put}) {
      console.log('payload', payload);
    ...
    // 关键字
    * couponSubtractionPushDisCodeSeller({payload, callback}, {put}) {
      yield put({
    ...
    // 每条页数
    * couponSubtractionPushShowSize({payload, callback}, {put}) {
      yield put({
        type: 'pushShowSize',
        payload,
      });
    ...
    // 当前页码
    * couponSubtractionPushShowNum({payload, callback}, {put}) {
      yield put({
        type: 'pushShowNum',
        payload,
      });
